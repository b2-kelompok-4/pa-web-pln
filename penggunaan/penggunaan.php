...
<form action="" method="POST">
	...
	<?php if (isset($_POST['serch'])) {
		...
		<a href="admin.php?page=addMakhir&meter=<?php echo $_POST['serch'] ?>" class="btn-xs btn-hijau" style="margin-left: 40px">Tambahkan Penggunaan</a>
	...
	if (isset($_POST['serch'])) {
		...
		if (mysqli_num_rows($query) == 0) { ?>
			...
			if (mysqli_num_rows($pgn) == 0) { ?>
				<script>
					window.alert('Belum Ada Penggunaan, Input Baru')
					window.location = 'admin.php?page=addPenggunaan&meter=<?php echo $_POST['serch'] ?>'
				</script>;
			<?php }
		}
			...
			<tr>
				...
				<td>
					...
					<?php elseif ($test['id_penggunaan'] == $pggn['id_penggunaan']) : ?>
						<a href="admin.php?page=editPenggunaan&id=<?php echo $pggn['id_penggunaan'] ?>&meter=<?php echo $_POST['serch'] ?>&bulan=<?php echo $pggn['bulan'] ?>&tahun=<?php echo $pggn['tahun'] ?>&mawal=<?php echo $pggn['meter_awal'] ?>&makhir=<?php echo $pggn['meter_akhir'] ?>" class="btn-xs btn-kuning">Ubah Meter Akhir</a>
					<?php else : ?>

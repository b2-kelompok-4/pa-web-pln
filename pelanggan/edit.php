...
<form action="" method="POST">
    <label for="kwh">No. KWH</label>
    <input type="text" name="kwh" class="form-input" id="kwh" value="<?php echo $row['no_meter'] ?>">
    ...
    <select name="tarif" id="tarif" class="form-input">
        <?php $sql = mysqli_query($conn, "SELECT * FROM tarif"); while($result = mysqli_fetch_assoc($sql)){ ?>
            <option value="<?php echo $result['id_tarif'] ?>"<?php if($row['id_tarif']==$result['id_tarif']){echo "selected='selected'";} ?>><?php echo $result['daya'] ?></option>
        <?php } ?>
    </select>
